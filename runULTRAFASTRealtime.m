...
%         GDcomp = GDref.^2-GDcalib.^2;
        GDcomp = medfilt1(GDref-GDcalib, winN);
%         GDcomp = medfilt1(GDref.^2-GDcalib.^2, winN);
...
%         GDcomp = GDref-GDzero;
%         GDcomp = GDref.^2-GDzero.^2;
        GDcomp = medfilt1(GDref-GDzero, winN);
%         GDcomp = medfilt1(GDref.^2-GDzero.^2, winN);
...
%         tcur = etime(clock, tstart);
    ...
    for i=1:Nt
%         GDcomp = GDref-GDs(:, i);
%         GDcomp = GDref.^2-GDs(:, i).^2;
        GDcomp = medfilt1(GDref-GDs(:, i), winN);
%         GDcomp = medfilt1(GDref.^2-GDs(:, i).^2, winN);
    ...
    csvwrite([prefix  'GDplots' suffix], [t0s f0s ps]);
